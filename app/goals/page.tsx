...
const goalPresets = [
  ...
  { name: "Home Down Payment", icon: "M2.25 12l8.954-8.955a1.126 1.126 0 011.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25", color: "#06b6d4" },
    ...
    <Modal open={open} onClose={onClose} title="New Goal" description="Set a savings target to work toward" wide>
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {goalPresets.map((preset, i) => (
              <button
                ...
                className={`glass-surface flex items-center gap-2 rounded-xl p-2.5 text-xs transition-all duration-200 ${
                  selected === i
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" className="flex-1 rounded-xl bg-linear-to-r from-cyan-500 to-emerald-500 px-4 py-2.5 text-sm font-medium text-white transition-all duration-200 hover:opacity-90">Create goal</button>
        </div>
      </form>
    </Modal>
  );
      ...
      <div className="space-y-6">
        ...
        {goals.length > 0 ? (
          <>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="glass-card p-5">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider">Overall progress</p>
                ...
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-zinc-800/50">
                  <div className="h-full rounded-full bg-linear-to-r from-cyan-500 to-emerald-500 transition-all duration-500" style={{ width: `${overallPercent}%` }} />
                </div>
              </div>
              <div className="glass-card p-5">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider">Total saved</p>
                <p className="mt-2 text-2xl font-bold tracking-tight text-cyan-400">{formatCurrency(totalSaved)}</p>
                <p className="mt-1 text-xs text-zinc-500">across {goals.length} goals</p>
              </div>
              <div className="glass-card p-5">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider">Still needed</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {goals.map((goal) => {
                ...
                return (
                  <div key={goal.id} className="glass-card group p-5 transition-all duration-200 hover:scale-[1.02]">
          ...
          <div className="glass-card flex flex-col items-center justify-center border-dashed py-16">
            ...
            <button type="button" onClick={() => setAddOpen(true)} className="mt-4 rounded-xl bg-linear-to-r from-cyan-500 to-emerald-500 px-4 py-2 text-sm font-medium text-white transition-all duration-200 hover:opacity-90">Create goal</button>
          </div>
        )}
      </div>
